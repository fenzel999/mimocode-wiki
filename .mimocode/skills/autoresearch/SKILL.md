...
Check if `references/program.md` exists. If it does, read it to load research objectives and constraints. Otherwise, use default settings:
- Max rounds: 3
- Max pages per session: 15
- Source preference: official docs, academic papers, reputable sources
...
Before each fetch and before writing fetched content to the vault, apply these guards:
...
**2. Content sanitization before writing fetched HTML into a wiki page.** Before any write to `wiki/sources/<source>.md`:
- Strip `<script>`, `<iframe>`, `<style>` tags and their contents
- Escape `[[` and `]]` in the source body
- Truncate fetched bodies to ~50KB to avoid context blowout

**3. Failure mode.** If a fetch fails (timeout, 4xx/5xx, content too large), log the URL + reason to `wiki/log.md` and continue the loop. Do NOT abort the whole run.
