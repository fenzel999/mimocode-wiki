...
Use direct file system access. Read and write files using the read/write tools with absolute paths.
...
3. **Create** source summary in `wiki/sources/`. Use frontmatter with type, title, created, updated, tags, status, related, sources.
...
- **Source files under `.raw/` are immutable.** Do not modify the files that users drop there.
